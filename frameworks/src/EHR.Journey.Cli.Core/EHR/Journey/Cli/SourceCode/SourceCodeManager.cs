namespace EHR.Journey.Cli.SourceCode;

public class SourceCodeManager : ITransientDependency, ISourceCodeManager
{
    private readonly ILogger<SourceCodeManager> _logger;
    private readonly IJourneyManager _abpProManager;
    private readonly JourneyCliOptions _cliOptions;

    public SourceCodeManager(ILogger<SourceCodeManager> logger, IOptions<Options.JourneyCliOptions> options, IJourneyManager abpProManager)
    {
        _logger = logger;
    ...
    public async Task<TemplateFile> GetAsync(string version)
    {
        ...
        _logger.LogInformation($"EHR Journey Version:{version}");
