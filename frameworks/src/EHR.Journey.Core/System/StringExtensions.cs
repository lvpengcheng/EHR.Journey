...
public static class StringExtensions
{
    ...
    public static string AsFormat(this string str, params object[] value)
    {
        return string.Format(str, value);
    }

    public static string AsToSqlString(this object o)
    {
        ...
        if (o.GetType().IsGenericType && o.GetType().GetGenericTypeDefinition() == typeof(Nullable<>))
        {
            ...
            var nullableConverter = new NullableConverter(objectType);
            objectType = nullableConverter.UnderlyingType;
            o = Convert.ChangeType(o, objectType);
        }
        if (o.GetType() == typeof(DateTime))
        {
            return ((DateTime)o).ToString("'yyyy-MM-dd HH:mm:ss.fff'");
        }
        else if (o.GetType() == typeof(int) || o.GetType() == typeof(decimal) || o.GetType() == typeof(float) || o.GetType() == typeof(double) || o.GetType() == typeof(long) || o.GetType() == typeof(short))
        {
            return Convert.ToString(o);
        }
        else
        {
            return "'{0}'".AsFormat(Convert.ToString(o));
        }
    }
    ...
    /// <returns></returns>
    public static string AsJoin<T>(this IEnumerable<T> t, string separator)
    {
    ...
    /// <param name="separator"></param>
    /// <returns></returns>
    public static string AsJoin<T>(this List<T> t, string separator)
    {
        if (separator == null) separator = "";
        return String.Join(separator, t);
    }
    ...
    /// <returns></returns>
    public static string ToClearSqlInj(this object obj, bool isStrictly)
    {
        ...
        Func<string, string> keyFilter = (s) =>
        {
            string[] key = { "sysdatabases", "master", "db_name()", "--", "backup%20database", "sysobjects", "bcp", "sysadmin", "dbcreator", "diskadmin", "processadmin", "serveradmin", "setupadmin", "securityadmin", "bulkadmin", "sp_addlogin", "sp_password", "sp_addsrvrolemember", "sp_configure", "sp_addextendedproc", "cmd.exe", "execsp_oamethod", "execsp_oacreate", "reverse", "=0x", "xp_regaddmultistring", "xp_regdeletekey", "xp_regdeletevalue", "xp_regenumvalues", "xp_regread", "xp_regremovemultistring", "xp_regwrite", "xp_cmdshell", "xp_fileexist", "xp_getfiledetails", "xp_dirtree", "xp_getnetname", "xp_availablemedia", "xp_subdirs", "sp_oacreate", "sp_oadestroy", "sp_oageterrorinfo", "sp_oagetproperty", "sp_oamethod", "sp_oasetproperty", "sp_oastop" };
            string[] reg = { "xp_", "insert ", "delete ", "update ", "drop ", "truncate", "create ", "drop " };
            string[] regStrictly = { "'", "sp_", "\"", "exec ", "select " };

            key.ToList().ForEach(d =>
            {
                s = s.Replace(d, "");
            });
            reg.ToList().ForEach(d =>
            {
                s = Regex.Replace(s, d, "");
            });
            if (isStrictly)
            {
                regStrictly.ToList().ForEach(d =>
                {
                    s = Regex.Replace(s, d, "");
                });
            }
    ...
    /// <returns></returns>
    public static string[] AsSplit(this string str, string splitString, bool isRemoveEmpty = true)
    {
        return str.Split(new string[] { splitString }, isRemoveEmpty ? StringSplitOptions.RemoveEmptyEntries : StringSplitOptions.None);
    }
    ...
    /// <returns></returns>
    public static List<string> AsSplit(this string str, IEnumerable<string> contains, StringSplitOptions stringSplitOptions = StringSplitOptions.None)
    {
        if (str.IsNullOrEmpty() || contains == null || !contains.Any())
            return new List<string> { str };

        return str.Split(contains.ToArray(), stringSplitOptions).ToList();
    }
