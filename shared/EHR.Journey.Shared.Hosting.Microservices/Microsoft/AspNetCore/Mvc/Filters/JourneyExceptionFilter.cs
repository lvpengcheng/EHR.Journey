...
public class JourneyExceptionFilter : AbpExceptionFilter
{
    ...
    private WrapResult<object> SimplifyMessage(ExceptionContext context)
    {
        ...
        var localizer = context.GetRequiredService<IStringLocalizer<JourneyLocalizationResource>>();
        switch (context.Exception)
        {
            case AbpAuthorizationException:
                result.SetFail(localizer["EHR.Journey:PermissionDenied"], "401");
                break;
            case AbpValidationException validationException:
                var errorMessage = localizer["EHR.Journey:ParameterValidationFailed"] + ";" + validationException.ValidationErrors.JoinAsString(";");
                result.SetFail(errorMessage, "400");
                break;
            case EntityNotFoundException:
                result.SetFail(localizer["EHR.Journey:EntityNotFound"], "506");
                break;
            case NotImplementedException:
                result.SetFail(localizer["EHR.Journey:Unimplemented"], "507");
                break;
            case DbUpdateConcurrencyException:
                result.SetFail(localizer["EHR.Journey:DbUpdateConcurrency"], "508");
                break;
            default:
            {
                if (context.Exception is IHasErrorCode codeException)
                {
                    var exceptionConverter = context.GetRequiredService<IJourneyExceptionConverter>();
